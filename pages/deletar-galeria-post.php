...
<form name="Form_del_user" action="" method="post">
    ...
    if (isset($_POST['sim']) && $_POST['sim'] == 'Sim') {
        $verificarGaleria = read('galerias', "WHERE id_post=$id");
        ...
        if (!delete('galerias', "id_post=$id")) {
            echo '<div class="msg ok borderradius5" style="width:280px;margin:20px auto;">Galeria deletada. Atualizando página...</div>';
            echo '<script type="text/javascript">setTimeout(function(){location = "' . SITE . '/painel.php?pagina=galeria-post";}, 2000)</script>';
        } else {
